      ...
      const urls = SITE_PATHS.flatMap((route) =>
        LOCALE_CODES.map((locale) => {
          ...
          const alternates = [
            ...LOCALE_CODES.map(
              (code) =>
                `    <xhtml:link rel="alternate" hreflang="${LOCALES[code].hreflang}" href="${loc(
                  localizedRoute(route, code)
                )}" />`
            ),
            `    <xhtml:link rel="alternate" hreflang="x-default" href="${loc(
              localizedRoute(route, DEFAULT_LOCALE)
            )}" />`,
          ].join('\n');
